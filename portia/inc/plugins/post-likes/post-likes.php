...
function like_scripts() {
	wp_enqueue_script( 'infinity_like_post', INFINITY_THEMEROOT .'/inc/plugins/post-likes/js/post-likes.js', array('jquery'), '1.0', 1 );
	wp_localize_script( 'infinity_like_post', 'ajax_var', array(
 ...
 * (3) Save like data
 */
add_action( 'wp_ajax_nopriv_infinity-post-like', 'infinity_post_like' );
add_action( 'wp_ajax_infinity-post-like', 'infinity_post_like' );
function infinity_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );
	
	if ( isset( $_POST['infinity_post_like'] ) ) {
